...
The browser checks team progress automatically every **60 seconds** while the page is visible and no dialog is open. Manual refresh and refreshes after a successful submission or review remain immediate, subject to any server-requested cooldown. With one continuously polling tab per person, 35 viewers generate about 50,400 scheduled progress requests per day, excluding page loads and other actions. Publish this interval change by committing and pushing the website; it does not require a Worker deployment or change team sign-in sessions. The earlier load simulations used a 30-second interval.
