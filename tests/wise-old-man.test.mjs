...
test('WOM coalesces concurrent checks, caches both metrics, and backs off after failures', async () => {
  let now=time, calls=0, fail=false;
  const tracker=new WiseOldMan({now:()=>now,fetch:async url=>{
    calls++; assert.match(url,/metrics=barrows_chests&metrics=mimic$/);
