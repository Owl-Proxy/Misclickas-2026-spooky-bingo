  ...
  for (const entry of entries) {
    if (entry.boss) {
      tiles.push({
        ...
        requirementsHeading: entry.completion_mode === 'any' ? 'Any one of the following:' : entry.completion_mode === 'all' && entry.target_drops.length > 1 ? 'All of the following:' : '',
...
for (const item of event.items) {
  tiles.push({
    ...
    requirementsHeading: item.completion_mode === 'any' ? 'Any one of the following:' : item.completion_mode === 'all' && (item.target_items || [item.item_name]).length > 1 ? 'All of the following:' : '',
